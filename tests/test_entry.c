...
static void test_entry_long_path(void) {
    ...
    char long_path[512];
    snprintf(long_path, sizeof(long_path), "%c/%s",
             czim_entry_get_namespace(e), czim_entry_get_path(e));

    char expected[512];
    snprintf(expected, sizeof(expected), "%c/%s",
             e->ns, e->path);

    TEST_ASSERT_EQUAL_STRING(expected, long_path);
    czim_entry_free(e);
    czim_archive_close(a);
}
